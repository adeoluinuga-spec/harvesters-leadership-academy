...
function Hero({
  ...
  const heroStats = [
    { label: "Leaders in scope", value: analytics ? fmt(analytics.totalLeaders) : "…" },
    { label: "Enrolled", value: analytics ? fmt(analytics.enrolledLeaders) : "…" },
    { label: "Completion", value: analytics ? `${analytics.completionRate}%` : "…" },
    { label: "Certificates", value: analytics ? fmt(analytics.certificates) : "…" },
  ];
...
export default function DirectionalDashboardPage() {
  ...
  const scope = useMemo(
    () => getLeadershipScope((role || "Directional Leader") as AcademyRole, campusId),
    [role, campusId]
  );
  ...
  return (
    ...
    >
      <DashboardShell searchPlaceholder="Search leaders, roles, follow-ups..." showDate>
        <Hero scope={scope} campusName={campusName || "Campus"} analytics={analytics} />

        <PersonalLearningLayer
          role={role || "Directional Leader"}
        />

        <OversightLayerIntro
          title={scope.dashboardTitle}
          description={`Live intelligence for ${scope.insightContext} within ${campusName || "your campus"}. Track enrolment, completion, role-tier performance, and leaders who need your follow-up.`}
          modules={[
            "Role-tier breakdown",
            "Enrolment tracking",
            "Completion pipeline",
          ...
          <motion.section variants={shellItem}>
            <div className="rounded-xl border border-dashed border-zinc-200 bg-white py-14 text-center">
              ...
              <p className="mt-1 text-xs text-zinc-400">
                Leaders in {campusName || "your campus"} with roles below yours will appear here once
                they have completed their onboarding profile.
              </p>
            </div>
          </motion.section>
        )}
